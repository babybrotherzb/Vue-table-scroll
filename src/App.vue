 ...
 <!-- 滚动条 -->
      ...
      <img style="left:210px;position:fixed;bottom:0px;" src="http://img.s.youfenba.com/material_thumb/74NGnHkeQs.png">  
          <div class="in_box" @mousedown="mouseDown" @mouseup="mouseUp" @mouseout="mouseUp"  :style="old_new" ref="in_box" > </div>
        <img style="position:fixed;bottom:0px;right:55px;" src="http://img.s.youfenba.com/material_thumb/WTzsMr6RSX.png">    
      </div>
  </div>
  </template>
      ...
      methods:{
        ...
        mouseDown(event) {
          ...
          this.$refs.in_box.addEventListener("mousemove", function(event) {
            ...
            _self.moveWidth = (1 - _self.windowWidth / 1300) * _self.windowWidth-parseFloat(_self.leftstr); //模拟滚动条可移动长度
            if (_self.f) {
              _self.old_new.left =
            ...
            document.querySelectorAll(".el-table__body-wrapper")[0].scrollLeft =
                _self.oldx - _self.newx <= -parseFloat(_self.leftstr)
                  ? 0
                  : _self.oldx - _self.newx >= _self.moveWidth
                    ?(_self.moveWidth + parseFloat(_self.leftstr))* (1300 / _self.windowWidth)
                    : (_self.oldx - _self.newx + parseFloat(_self.leftstr) ) * (1300 / _self.windowWidth); //实际滚动条偏移量
            }
          });
        },
        mouseUp() {
        ...
        ifmove() {
          if(this.$route.path=='/'){//防止window.onresize 滥用
          let that = this;
          that.windowWidth = document.body.clientWidth - 280;
          if (that.windowWidth < 1300) {
            document.querySelectorAll(".el-table__body-wrapper")[0].style.overflow =
              "hidden";
